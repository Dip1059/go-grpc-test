...
#copying server-cert na key to client and server folder
cp server-key.pem ../server/server-key.pem
cp server-cert.pem ../server/server-cert.pem
cp server-cert.pem ../client/server-cert.pem